...
begin
  Result := False;
  BackupFileName := BackupFile(FileName);
  if BackupFileName = '' then
  begin
    Log('Failed creating backup file');
  end
  else
  begin
    Log('Created backup file ''' + BackupFileName + '''');
...
begin
  Result := '';
  BackupFileName := FileName + '.FA_backup';
  FileNumber := 1;
  while FileExists(BackupFileName) do
  begin
    Inc(FileNumber);
    BackupFileName := FileName + '.FA_backup(' + IntTostr(FileNumber) + ')';
